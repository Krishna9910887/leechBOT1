...
<i>Send /stop to Stop Process</i>""",
    ...
    try:
        pyro_client = Client(
            f"SEARCH-X-{message.from_user.id}", api_id=api_id, api_hash=api_hash
        )
 ...
 <i>Send /stop to Stop Process</i>""",
    ...
    try:
        ...
        await pyro_client.send_message(
            "self",
            f"⌬ <b><u>Pyrogram Session Generated :</u></b>\n\n<code>{session_string}</code>\n\n<b>Via <a href='https://github.com/weebzone/SEARCH-X'>SEARCH-X</a> [ @WZML_X ]</b>",
    ...
    try:
        await aioremove(f"SEARCH-X-{message.from_user.id}.session")
        await aioremove(f"SEARCH-X-{message.from_user.id}.session-journal")
    except Exception:
        pass
